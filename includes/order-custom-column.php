    ...
    function aireset_custom_orders_list_column_content( $column, $post_id ) {
        if ( $column === 'order_number' ) {
            ...
            if ( $phone = $the_order->get_billing_phone() ) {
                ...
                echo '<br>' . esc_html__( 'Celular:', 'aireset-01-default-woo' ) . ' <a href="tel:' . esc_attr( $escaped_phone ) . '" target="_blank">' . esc_html( $escaped_phone ) . '</a>';
                ...
                $text = urlencode( sprintf( 
                    ...
                    esc_html__( "Olá %1\$s, tudo bem?\nVimos que você fez o pedido %2\$d que está como %3\$s. Conte para nós como podemos te ajudar?", 'aireset-01-default-woo' ), 
                    $the_order->get_billing_first_name(), 
                    $the_order->get_id(), 
                    $status 
                ));

                echo '<br>' . esc_html__( 'Whatsapp:', 'aireset-01-default-woo' ) . ' <a href="https://wa.me/55' . esc_attr( preg_replace( "/[^0-9]/", "", $escaped_phone ) ) . '?text=' . esc_attr( $text ) . '" target="_blank">' . esc_html( $escaped_phone ) . '</a>';
            }

            if ( $email = $the_order->get_billing_email() ) {
                // Escape email address
                $escaped_email = esc_html( $email );
                echo '<br>' . esc_html__( 'Email:', 'aireset-01-default-woo' ) . ' <a href="mailto:' . esc_attr( $escaped_email ) . '" target="_blank">' . esc_html( $escaped_email ) . '</a>';
            }
        }
    }
}
