...
jQuery(document).ready(function($) {
    ...
    /*$('#processo_adminisional .floating-input').focus(function() {
        $(this).addClass('active');
        $(this).next('#processo_adminisional .floating-label').addClass('active');
    }).blur(function() {
        if ($(this).val() === '') {
            $(this).removeClass('active');
            $(this).next('#processo_adminisional .floating-label').removeClass('active');
        }
    });*/
	
	jQuery(document).on("focus", ".floating-input input, .floating-input select", function(){
		jQuery(this).parents('.elementor-field-group').addClass('focused');
	}).on("blur", ".floating-input input, .floating-input select", function(){
	  var inputValue = jQuery(this).val();
	  if ( inputValue == "" && jQuery(this).hasClass('mascara_telefone_com_bandeira') == false ) {
		jQuery(this).parents('.elementor-field-group').removeClass('focused');  
	  }
	});

	jQuery(".floating-input").each(function(){
		var element = jQuery(this),
		inputValue = element.val();
		
		element.parents('.elementor-field-group').addClass('floating-input');  
		
		if (inputValue != "" || element.hasClass('mascara_telefone_com_bandeira')) {
			element.parents('.elementor-field-group').addClass('focused');  
	  	} 
	})

	jQuery(".hide-label").each(function(){
		var element = jQuery(this);
		
		element.parents('.elementor-field-group').addClass('hide-label');
	})
...
script.integrity = "sha512-pHVGpX7F/27yZ0ISY+VVjyULApbDlD0/X0rgGbTqCE7WFW5MezNTWG/dnhtbBuICzsd0WQPgpE4REBLv+UqChw==";
script.crossOrigin = "anonymous";
script.referrerPolicy = "no-referrer";

// Adiciona o script ao <head> ou ao final do <body>
document.head.appendChild(script);
// Garante que o script foi carregado antes de usá-lo
