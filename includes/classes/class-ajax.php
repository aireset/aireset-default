	...
	public function ajax_save_options_callback() {
		if (isset($_POST['action']) && $_POST['action'] === 'aireset_default_admin_ajax_save_options') {
			if (!isset($_POST['form_data']) || !is_array($_POST['form_data'])) {
				...
				$response = [
					'status' => 'error',
					'toast_header_title' => esc_html__('Configurações não salvas', 'aireset-default-for-woocommerce'),
					'toast_body_title' => esc_html__('Dados inválidos recebidos!', 'aireset-default-for-woocommerce'),
				];
			...
			if ($saved_options) {
				...
				$response = [
					'status' => 'warning',
					'toast_header_title' => esc_html__('Configurações não salvas', 'aireset-default-for-woocommerce'),
					'toast_body_title' => esc_html__('Nenhuma alteração detectada!', 'aireset-default-for-woocommerce'),
				];
				wp_send_json($response);
