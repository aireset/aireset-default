...
class Helpers {
	...
	public static function get_order_pay_btn_text( $order ) {
		return esc_html__( 'Pagar pelo pedido', 'aireset-default' ) . ' - ' . wc_price( $order->get_total() );
	}
	...
	public static function get_placeholder_input_values() {
		...
		foreach ( self::get_checkout_fields_on_admin()['billing'] as $field_id => $value ) {
			...
			$fields[$field_id] = array(
				...
				'placeholder_html' => sprintf( esc_html( '{{ %s }}', 'aireset-default' ), $placeholder_id ?? '' ),
				'description' => sprintf( esc_html( 'Para recuperar o valor de %s', 'aireset-default' ), isset( $value['label'] ) ? $value['label'] : '' ),
			);
		}
