( function($) {
						...
						if (response.status === 'success') {
							...
							$('.aireset-default-wrapper').before(`<div class="toast toast-save-options toast-success show">
								<div class="toast-header bg-success text-white">
									...
									<button class="btn-close btn-close-white ms-2 hide-toast" type="button" aria-label="${aireset_default_params.close_aria_label_notice}"></button>
								</div>
	 ...
	 */
	jQuery(document).ready( function($) {
		var step_1 = $('#aireset_default_step_1').sortable({
			connectWith: '#aireset_default_step_2',
			update: function(event, ui) {
				update_fields_priority(event, ui, '1');
			},
		});
	
		var step_2 = $('#aireset_default_step_2').sortable({
			connectWith: '#aireset_default_step_1',
			update: function(event, ui) {
				update_fields_priority(event, ui, '2');
			},
		});

		sort_fields_by_priority('aireset_default_step_1');
		sort_fields_by_priority('aireset_default_step_2');
		...
		$(document).on('click', '.exclude-field', function(e) {
			...
			var confirm_exclude_field = confirm(aireset_default_params.confirm_exclude_field);
		...
		$(document).on('click', '#fcw_add_new_field', function(e) {
			...
			var btn_height = btn.height();
			var priority = aireset_default_params.get_array_checkout_fields.length + 1;
					...
					try {
						if (response.status === 'success') {
							...
							if (existing_field.length) {
								...
								if (step === '1') {
									$('#aireset_default_step_1').append(response.field_html);
								} else if (step === '2') {
									$('#aireset_default_step_2').append(response.field_html);
								}
							}
		...
		$(document).on('click', '.exclude-option-select-live', function(e) {
			...
			var confirm_remove_option = confirm(aireset_default_params.confirm_remove_option);
			...
			var new_option = `<div id="new_select_option_live_preview" class="d-flex align-items-center justify-content-between mb-4">
				<div class="d-grid me-3">
					<div class="input-group mb-3">
						<span class="input-group-text w-fit">${aireset_default_params.new_option_value}</span>
						<input type="text" id="add_new_field_select_option_value_live" class="form-control input-control-wd-12" value="" placeholder="${aireset_default_params.placeholder_new_option_value}">
					</div>
					<div class="input-group">
						<span class="input-group-text w-fit">${aireset_default_params.new_option_title}</span>
						<input type="text" id="add_new_field_select_option_title_live" class="form-control input-control-wd-12" value="" placeholder="${aireset_default_params.placeholder_new_option_title}">
					</div>
				</div>
					...
					success: function(response) {
						try {
							if (response.status === 'success') {
								...
								$('.drop-file-license-key').after('<div class="text-danger mt-2"><p>'+ aireset_default_params.invalid_file +'</p></div>');
				...
				success: function(response) {
					try {
						if ( response.status === 'success' && response.reload === true ) {
							...
							btn.html(btn_html);
							btn.before('<div class="alert alert-danger me-3"><svg class="icon-danger me-2" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"></path><path d="M11 11h2v6h-2zm0-4h2v2h-2z"></path></svg>'+ aireset_default_params.font_exists +'</div>');
						}
					} catch (error) {
						console.log(error);
				...
				success: function(response) {
					try {
						if ( response.status === 'success' ) {
							...
							$('#aireset_default_email_providers').append(`<li class="list-group-item d-flex align-items-center justify-content-between" data-provider="${response.new_provider}">
	...
	jQuery(document).ready( function($) {
		$('#aireset_default_deactive_license').on('click', function(e) {
			...
			var confirm_deactivate_license = confirm(aireset_default_params.confirm_deactivate_license);
