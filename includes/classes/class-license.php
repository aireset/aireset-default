...
class License {
    ...
    private $fcw_product_base = 'aireset-checkout-for-woocommerce';
    ...
    private $email_address = AIRESET_DEFAULT_ADMIN_EMAIL;
    ...
    public function __construct( $plugin_base_file = '' ) {
        // $license_key = get_option('aireset_checkout_license_key');
        ...
        // add_action( 'aireset_checkout_display_admin_notices', array( $this, 'display_modal_license_require' ) );
    }
    ...
    //     $plugin_log_file = 'aireset-checkout-for-woocommerce-log';
    //     $logger->info('(aireset Checkout para WooCommerce) Response encrypted: ' . print_r( $encrypted, true ), array('source' => $plugin_log_file));
    ...
    //         if ( $plaintext === false ) {
    //             $logger->info('(aireset Checkout para WooCommerce) Falha na descriptografia. Input: $encrypted: ' . print_r( $plaintext, true ), array('source' => $plugin_log_file));
    ...
    //     } else {
    //         $logger->info('(aireset Checkout para WooCommerce) A entrada para decrypt não é uma string. Tipo: ' . gettype( $encrypted ), array('source' => $plugin_log_file));
    ...
    //     if ( get_option('aireset_checkout_alternative_license') === 'active' ) {
    //         return;
    //     }
    ...
    //         $logger->info('(aireset Checkout para WooCommerce) Response: ' . print_r( $response, true ), array('source' => $plugin_log_file));
    ...
    //             $logger->info('(aireset Checkout para WooCommerce) Decrypted response: ' . print_r( $decrypted_response, true ), array('source' => $plugin_log_file));

    //             if ( empty( $decrypted_response ) ) {
    //                 update_option( 'aireset_checkout_alternative_license_activation', 'yes' );

    //                 // Handle decryption failure
    //                 $decryption_error = new \stdClass();
    //                 $decryption_error->status = false;
    //                 $decryption_error->msg = __( 'Ocorreu um erro na conexão com o servidor de verificação de licenças. Verifique o erro nos logs do WooCommerce.', 'aireset-checkout-for-woocommerce' );
    ...
    //         $logger->info('(aireset Checkout para WooCommerce) Response decoded: ' . print_r( $decoded_response, true ), array('source' => $plugin_log_file));

    //         if ( json_last_error() !== JSON_ERROR_NONE ) {
    //             // Handle JSON decoding error
    //             $json_error = new \stdClass();
    //             $json_error->status = false;
    //             $json_error->msg = sprintf( __( 'Erro JSON: %s', 'aireset-checkout-for-woocommerce' ), json_last_error_msg() );
    ...
    //     $unknown_response->msg = __( 'Resposta desconhecida', 'aireset-checkout-for-woocommerce' );
    ...
    //     $transient_name = 'aireset_checkout_api_request_cache';
    //     $cached_response = get_transient( $transient_name );

    //     if ( false === $cached_response ) {
    //         $response = new \stdClass();
    //         $response->status = false;
    //         $response->msg = __( 'Resposta vazia.', 'aireset-checkout-for-woocommerce' );
    ...
    //             $logger->info('(aireset Checkout para WooCommerce) Request response: ' . print_r( $server_response, true ), array('source' => $plugin_log_file));
    ...
    //                         $error = __( 'Erro cURL 35: Problema de comunicação SSL/TLS.', 'aireset-checkout-for-woocommerce' );
    //                     } else {
    ...
    //             $response->msg = __( 'A extensão cURL está faltando.', 'aireset-checkout-for-woocommerce' );
    //             $response->status = false;
    //             $response->data = NULL;
    //             $response->is_request_error = true;
    //         } else {
    ...
    //                     $error = __( 'Erro cURL 35: Problema de comunicação SSL/TLS.', 'aireset-checkout-for-woocommerce' );
    //                 } else {
    //                     $response->msg = sprintf( __( 'Erro cURL: %s', 'aireset-checkout-for-woocommerce' ), $error_message );
    //                 }
    //             }
    ...
    //             update_option('aireset_checkout_license_response_object', $response);

    //             $logger = wc_get_logger();
    //             $plugin_log_file = 'aireset-checkout-for-woocommerce-log';
    //             $logger->info('(aireset Checkout para WooCommerce) Deactive response object: ' . print_r( $response, true ), array('source' => $plugin_log_file));
    ...
    /* final function check_license_object( $purchase_key, &$error = '', &$response_object = null ) {
        ...
        $transient_name = 'aireset_checkout_api_response_cache';
        $cached_response = get_transient( $transient_name );
            ...
            if ( empty( $response->code ) ) {
                if ( ! empty( $response->status ) ) {
                    if ( ! empty( $response->data ) ) {
                        ...
                        update_option( 'aireset_checkout_license_response_object', $licenseObj );
                        ...
                        $error = __( 'Dados inválidos.', 'aireset-checkout-for-woocommerce' );
                    }
                } else {
                    $error = $response->msg;
                }
    ...
    public function licenses_api_connection() {
        ...
        if ( isset( $_POST['aireset_checkout_active_license'] ) ) {
            // clear response cache first
            delete_transient('aireset_checkout_api_request_cache');
            delete_transient('aireset_checkout_api_response_cache');
            delete_transient('aireset_checkout_license_status_cached');
    
            $license_key = ! empty( $_POST['aireset_checkout_license_key'] ) ? $_POST['aireset_checkout_license_key'] : '';
            update_option( 'aireset_checkout_license_key', $license_key ) || add_option('aireset_checkout_license_key', $license_key );
            update_option( 'aireset_checkout_temp_license_key', $license_key ) || add_option('aireset_checkout_temp_license_key', $license_key );
    
            // Check on the server if the license is valid and update responses and options
            if ( self::check_license( $license_key, $this->license_message, $this->response_obj, aireset_CHECKOUT_FILE ) ) {
                if ( $this->response_obj && $this->response_obj->is_valid ) {
                    update_option( 'aireset_checkout_license_status', 'valid' );
                    delete_option('aireset_checkout_temp_license_key');
                    delete_option('aireset_checkout_alternative_license_activation');
                } else {
                    update_option( 'aireset_checkout_license_status', 'invalid' );
                }
        
                if ( isset( $_POST['aireset_checkout_active_license'] ) && self::is_valid() ) {
                    add_action( 'aireset_checkout_display_admin_notices', array( $this, 'activated_license_notice' ) );
                }
            } else {
                if ( ! empty( $license_key ) && ! empty( $this->license_message ) ) {
                    add_action( 'aireset_checkout_display_admin_notices', array( $this, 'display_license_messages' ) );
                }
            }
        }
    }
    ...
    public function alternative_activation_process() {
        $decrypted_license_data = get_option('aireset_checkout_alternative_license_decrypted');
        ...
        if ( $this_domain !== $license_data_array->site_domain ) {
            add_action( 'aireset_checkout_display_admin_notices', array( $this, 'not_allowed_site_notice' ) );
        ...
        if ( ! in_array( $license_data_array->selected_product, $allowed_products ) ) {
            add_action( 'aireset_checkout_display_admin_notices', array( $this, 'not_allowed_product_notice' ) );
        ...
        if ( $this_domain === $license_data_array->site_domain ) {
            ...
            $obj->app_version = AIRESET_CHECKOUT_VERSION;
            ...
            update_option( 'aireset_checkout_alternative_license', 'active' );
            update_option( 'aireset_checkout_license_response_object', $obj );
            update_option( 'aireset_checkout_license_key', $obj->license_key );
            update_option( 'aireset_checkout_license_status', 'valid' );
            delete_option('aireset_checkout_alternative_license_decrypted');
    ...
    public static function is_valid() {
        // $cached_result = get_transient('aireset_checkout_license_status_cached');
        ...
        // $object_query = get_option('aireset_checkout_license_response_object');

        // if ( is_object( $object_query ) && ! empty( $object_query ) && isset( $object_query->is_valid )  ) {
        //     // set response cache for 24h
        //     set_transient('aireset_checkout_license_status_cached', true, 86400);

            return true;
        // } else {
        //     set_transient('aireset_checkout_license_status_cached', false, 86400);
        //     update_option( 'aireset_checkout_license_status', 'invalid' );
    ...
    //     } else {
    //       return esc_html__( 'Não disponível', 'aireset-checkout-for-woocommerce' );
    //     }
    // }
    ...
    //             return esc_html__( 'Nunca expira', 'aireset-checkout-for-woocommerce' );
    //         } else {
    //             if ( strtotime( $object_query->expire_date ) < time() ) {
    //                 $object_query->is_valid = false;

    //                 update_option( 'aireset_checkout_license_response_object', $object_query );
    //                 update_option( 'aireset_checkout_license_status', 'invalid' );
    //                 delete_option('aireset_checkout_license_response_object');

    //                 return esc_html__( 'Licença expirada', 'aireset-checkout-for-woocommerce' );
    //             }
                 ...
                 <div class="popup-content popup-sm">
                     <div class="popup-body">
                         <div class="d-flex flex-column align-items-center p-4">
                             ...
                             <h5 class="text-center mb-2 mt-3"><?php echo esc_html__('Este recurso está disponível na versão Pro', 'aireset-checkout-for-woocommerce'); ?></h5>
                             <span class="title-hightlight mt-2 mb-3"></span>
                             <span class="text-muted fs-lg p-3"><?php echo esc_html__( 'Uma licença permite que você desbloqueie todos os recursos Pro que o plugin tem a oferecer.', 'aireset-checkout-for-woocommerce' ) ?></span>
                         </div>
    ...
    /* public function activated_license_notice() {
        ...
        <div class="toast update-notice-wci show">
            <div class="toast-header bg-success text-white">
                ...
                <span class="me-auto"><?php echo esc_html__( 'Licença ativada com sucesso!', 'aireset-checkout-for-woocommerce' ); ?></span>
                <button class="btn-close btn-close-white ms-2 hide-toast" type="button" data-bs-dismiss="toast" aria-label="Close"></button>
            </div>
            <div class="toast-body"><?php echo esc_html__( 'Todos os recursos da versão Pro agora estão ativos!', 'aireset-checkout-for-woocommerce' ); ?></div>
        </div>
        <?php
    } */
        ...
        if ( ! empty( $this->license_message ) ) : ?>
            <div class="toast toast-danger show">
                <div class="toast-header bg-danger text-white">
                    ...
                    <span class="me-auto"><?php echo esc_html__( 'Ops! Ocorreu um erro.', 'aireset-checkout-for-woocommerce' ); ?></span>
                    <button class="btn-close btn-close-white ms-2 hide-toast" type="button" data-bs-dismiss="toast" aria-label="Close"></button>
                </div>
                <div class="toast-body"><?php echo esc_html__( $this->license_message, 'aireset-checkout-for-woocommerce' ); ?></div>
            </div>
        <?php endif;
    } */
    ...
    /* public function not_allowed_site_notice() {
        ...
        <div class="toast toast-danger show">
            ...
            <div class="toast-body"><?php echo esc_html__( 'O domínio de ativação não é permitido.', 'aireset-checkout-for-woocommerce' ) ?></div>
        </div>
        <?php
    }
 */
    ...
    /* public function not_allowed_product_notice() {
        ...
        <div class="toast toast-danger show">
            ...
            <div class="toast-body"><?php echo esc_html__( 'A licença informada não é permitida para este produto.', 'aireset-checkout-for-woocommerce' ) ?></div>
        </div>
        <?php
    } */
}
